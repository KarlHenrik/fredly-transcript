...
function AudioPlayerComponent() {
  ...
  const handleKeyPress = (event: KeyboardEvent): void => {
    ...
    if (
      !audioPlayerRefCurrent ||
      activeElement?.classList.contains("SpeakerInput") ||
      activeElement?.classList.contains("Quote")
    ) {
      return;
    }
    ...
    switch (event.key) {
      ...
      case "r": // 'R' key: rewind 10 seconds
        audioPlayerRefCurrent.currentTime = Math.max(
          0,
          audioPlayerRefCurrent.currentTime - 10
        );
        break;
      case "f":
        audioPlayerRefCurrent.currentTime = Math.min(
          audioPlayerRefCurrent.duration,
          audioPlayerRefCurrent.currentTime + 10
        );
        break;
      case "t":
      ...
      {!audioFileName && (
        <>
          ...
          <label className="buttonAction" htmlFor="audioFileInput">
            <FontAwesomeIcon className="symbol" icon={faFileAudio} /> Select
            Audio
          </label>
        </>
      )}
